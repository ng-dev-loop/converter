...
func (t *Table2Struct) getColumns(table ...string) (tableColumns map[string][]column, err error) {
	...
	for rows.Next() {
		...
		isNullEnable := strIsNullEnable=="YES"
		isKey := strIsKey=="PRI"

		var tmpTag = ""
		{
			tmpTag += fmt.Sprintf("`%v:\"", t.tagKey)
			if col.Tag != "-" {
				tmpTag += fmt.Sprintf("column(%v);", col.Tag)

				if col.Type == "string" && columnCharMaxLen.Valid {
					tmpTag += fmt.Sprintf("size(%v);", columnCharMaxLen.Int64)
				}

				if isNullEnable {
					tmpTag += fmt.Sprintf("null;")
				}

				if isKey {
					tmpTag += fmt.Sprintf("pk;")
				}

				if tmpTag[len(tmpTag) - 1] == ';' {
					tmpTag = tmpTag[0 : len(tmpTag) - 1]
				}
			} else {
				tmpTag += fmt.Sprintf("%v", col.Tag)
			}

			tmpTag += fmt.Sprintf("\"")

			if t.enableJsonTag {
				tmpTag += fmt.Sprintf(" json:\"%v\"`", col.Tag)
			} else {
				tmpTag += fmt.Sprintf("`")
			}
		}
